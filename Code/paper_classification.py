# Pytorch script for paper classification
...
from models import Year_Classifier
...
def load_data(hparams):
    ...
    #labels, data = create_labels_year_prof(data)
    ...
    #da = pre_tokenize_abstract(data[:20])
    #print(da[0,4])
...
def train_model(hparams, train_data, val_data, test_data, opt, X_test, y_test):
    ...
    # to keep track of effective batch size
    hparams["effective_batch_size"] = hparams["acc_grad"]*hparams["batch_size"]
    ...
    classifier = Year_Classifier(hparams, bert_t, bert_a, cnn, cnn_fig, hparams["id2tag"], train_data, val_data, test_data)

    classifier = classifier.to(device)
    early_stop_callback = EarlyStopping(monitor="val_loss", min_delta=0.00, patience=5, verbose=False, mode="min")
    best_checkpoint = ModelCheckpoint(monitor='val_f1_macro', save_top_k=1, mode="max")
    trainer = None
    logger = TensorBoardLogger("year_logs")
    trainer = pl.Trainer(
    ...
    with open('Year_Inference.txt', 'a') as f:
        f.write(str(pred)+"\n")
        f.write(str(tar)+"\n")
...
def parser_init():
    ...
    parser.add_argument('--bdrop', type=float, required=False, default=0.2,
    ...
    parser.add_argument('--reg', type=float, required=False, default=1e-10,
                        help='Weight decay')
    parser.add_argument('--gamma', type=float, required=False, default=1,
                        help='Gamma value for Exponential LR-Decay')
    parser.add_argument('--acc_grad', type=int, required=False, default=16,
